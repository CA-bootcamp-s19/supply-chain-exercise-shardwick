...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
   ...
   */
  modifier forSale(uint sku){ require(items[sku].state == State.ForSale && items[sku].price > 0); _;}
  ...
  function shipItem(uint sku)
    public sold(sku) verifyCaller(address(items[sku].seller))
  {
  ...
  function receiveItem(uint sku)
    public shipped(sku) verifyCaller(address(items[sku].buyer))
  {
